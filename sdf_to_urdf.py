...
def parsebaselink(base_link: Element):
    ...
    iner.append(Element('inertia', {'ixx': ixx, 'ixy': ixy, 'ixz': ixz, 'iyy': iyy, 'iyz': iyz, 'izz': izz}))
...
def parserotor(rotor):
    ...
    for visual in visuals:
        ...
        vis = SubElement(linkel, 'visual', {'name': visual.attrib.get('name', 'Nome non trovato')})
        vis.append(Element('origin', {'rpy': rpy, 'xyz': xyz}))
        geom = SubElement(vis, 'geometry')
        geom.append(Element('mesh', {'filename': filename, 'scale': scale}))
...
def parsejoint(joint: Element, root: Element):
    ...
    newjoint = Element('joint', {'name': joint.attrib["name"], 'type': joint.attrib["type"]})
    ...
    if axis is not None:
        ...
        if xyz is not None:
            ...
            logging.warning(f"Joint '{joint.attrib['name']}' axis is missing the 'xyz' element.")
